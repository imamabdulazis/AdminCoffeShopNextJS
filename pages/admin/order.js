...
export default function OrderPage({ color = "light" }) {
  ...
  const updateOrder = () => {
    setloading(true);
    fetch(`/api/v1/new_orders/status/${orderId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: "Bearer " + window.localStorage.getItem("token"),
      ...
      body: JSON.stringify({
        status: orderStatus == 1 ? "Proses" : "Selesai",
      }),
    })
      .then((res) => res.json())
      .then((res) => {
  ...
  const [columns, setColumns] = useState([
    ...
    { title: "NO_TRANSAKSI", field: "no_transaction", editable: "never" },
    { title: "JUMLAH", field: "order_items.length", editable: "never" },
    {
      title: "ORDER STATUS",
      editable: "never",
      field: "status",
      render: (rowData) => (
        <select
          ...
          onChange={(e) => changeOrderStatus(e, rowData)}
        >
  ...
  const changeOrderStatus = (event, rowData) => {
    setOrderStatus(
      event.target.value != null ? event.target.value : rowData.id
    );
